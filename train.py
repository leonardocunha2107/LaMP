...
from lamp.SubLayers import PLoss
...
def train_epoch(model,train_data, crit, optimizer,adv_optimizer,epoch,data_dict,opt,logger):
    ...
    for batch in tqdm(train_data, mininterval=0.5,desc='(Training)', leave=False):
            ...
            if opt.binary_relevance:
                ...
                # Non Binary Outputs
                optimizer.zero_grad()
                pred,enc_output,_,dec_output2 = model(src,adj,tgt,None,int_preds=opt.int_preds,return_attns=True)                                               
                dec_self_attn,dec_enc_attn=dec_output2
                                
                logger.push_attentions(dec_self_attn)
